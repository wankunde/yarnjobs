...
hive -e "load data local inpath '/tmp/yarnjobs/jobs/$1' overwrite into table tmp.jobs partition(dt=$1);"
hive -e "load data local inpath '/tmp/yarnjobs/conf/$1' overwrite into table tmp.jobconf partition(dt=$1);"
fi
rm -rf /tmp/yarnjobs/apps/$1 /tmp/yarnjobs/jobs/$1 /tmp/yarnjobs/conf/$1